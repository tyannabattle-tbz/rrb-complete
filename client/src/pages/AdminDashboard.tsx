...
export default function AdminDashboard() {
  ...
  const { data: stats, isLoading: statsLoading } = trpc.admin.admin.getDashboardStats.useQuery(undefined, {
    refetchInterval: refreshInterval,
  });

  // Fetch system health
  const { data: health, isLoading: healthLoading } = trpc.admin.admin.getSystemHealth.useQuery(undefined, {
    refetchInterval: refreshInterval,
  });

  // Fetch alerts
  const { data: alerts, refetch: refetchAlerts } = trpc.admin.admin.getAlerts.useQuery({ status: "active" });

  // Mutations
  const acknowledgeAlertMutation = trpc.admin.admin.acknowledgeAlert.useMutation({
    onSuccess: () => refetchAlerts(),
  });

  const resolveAlertMutation = trpc.admin.admin.resolveAlert.useMutation({
    onSuccess: () => refetchAlerts(),
  });

  const { mutateAsync: exportReport, isPending: isExporting } = trpc.admin.admin.exportReport.useMutation();
