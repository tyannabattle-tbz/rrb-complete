...
export default function CostOptimizationDashboard() {
  ...
  // Fetch cost data
  const { data: costAnalysis } = trpc.costOptimization.getCostAnalysis.useQuery({
    startDate: new Date(startDate),
    endDate: new Date(endDate),
  });

  const { data: costByTemplate } = trpc.costOptimization.getCostByTemplate.useQuery({
    startDate: new Date(startDate),
    endDate: new Date(endDate),
  });

  const { data: recommendations } = trpc.costOptimization.getOptimizationRecommendations.useQuery({
    startDate: new Date(startDate),
    endDate: new Date(endDate),
  });

  const { data: efficiency } = trpc.costOptimization.getResourceEfficiency.useQuery({
    startDate: new Date(startDate),
    endDate: new Date(endDate),
  });

  const { data: forecast } = trpc.costOptimization.getCostForecast.useQuery({
    days: 7,
  });
