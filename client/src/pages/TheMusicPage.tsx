  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 p-6">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-12">
          <h1 className="text-5xl font-bold text-white mb-4">The Music</h1>
              ...
              >
                ...
                <div className="p-4">
                  <h3 className={`font-bold text-lg mb-2 ${
                    selectedAlbum === album.id ? 'text-white' : 'text-white'
                  }`}>
                    {album.title}
                  </h3>
                  <p className={`text-sm ${
                    selectedAlbum === album.id ? 'text-amber-100' : 'text-slate-400'
                  }`}>
                    {album.releaseYear}
                  </p>
                </div>
          ...
          <div className="space-y-2">
            {songs?.map((song, index) => (
              <Card
                ...
                onClick={() => playMutation.mutate({ songId: song.id })}
              >
                <div className="flex items-center gap-4">
                  <div className="text-slate-500 font-mono w-8 text-right">{index + 1}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {credits.map((credit, index) => (
                <Card key={index} className="bg-slate-800 border-slate-700 p-6">
                  ...
                  {credit.description && (
                    <p className="text-sm text-slate-400 mt-2">{credit.description}</p>
                  )}
                </Card>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {awards.map((award, index) => (
                <Card key={index} className="bg-gradient-to-br from-amber-900/30 to-amber-800/20 border-amber-700/50 p-6">
                  ...
                  {award.description && (
                    <p className="text-sm text-slate-400 mt-3">{award.description}</p>
                  )}
                </Card>
              ))}
            </div>
