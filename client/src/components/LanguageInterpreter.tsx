...
export default function LanguageInterpreter({
  ...
  // Sync target language when prop changes (e.g., from quick-switch)
  useEffect(() => {
  ...
  const exportTranscriptPDF = useCallback(() => {
    if (transcript.length === 0) { toast.error('No transcript to export'); return; }
    ...
    let html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Conference Transcript</title>
    <style>
      @page { margin: 1in; }
      ...
      .header h1 { font-size: 24px; color: #1e1b4b; margin: 0 0 8px; }
      .header .meta { font-size: 13px; color: #64748b; }
      .entry { margin-bottom: 20px; padding: 12px 16px; border-left: 3px solid #6366f1; background: #f8fafc; border-radius: 0 8px 8px 0; }
      .entry .time { font-size: 11px; color: #94a3b8; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }
      .entry .speaker { font-size: 12px; color: #6366f1; font-weight: 600; margin-top: 4px; }
      .entry .original { font-size: 14px; color: #334155; margin-top: 6px; }
      .entry .translated { font-size: 14px; color: #1e40af; margin-top: 4px; font-style: italic; }
      .footer { margin-top: 40px; padding-top: 16px; border-top: 1px solid #e2e8f0; font-size: 11px; color: #94a3b8; text-align: center; }
    </style></head><body>
    <div class="header">
      <h1>\u{1F30D} Conference Transcript</h1>
      <div class="meta">${contextLabel ? `<strong>${contextLabel}</strong> &bull; ` : ''}${srcName} \u2192 ${tgtName} &bull; ${date} at ${time} &bull; ${transcript.length} entries</div>
    </div>`;

    transcript.forEach(e => {
      const t = e.timestamp.toLocaleTimeString();
      const sLang = LANGUAGES.find(l => l.code === e.sourceLang);
      const tLang = LANGUAGES.find(l => l.code === e.targetLang);
      html += `<div class="entry">
        <div class="time">${t} &bull; ${sLang?.flag || ''} ${sLang?.name || e.sourceLang} \u2192 ${tLang?.flag || ''} ${tLang?.name || e.targetLang}</div>
        <div class="original">${e.original}</div>
        <div class="translated">${e.translated}</div>
      </div>`;
    });

    html += `<div class="footer">Generated by QUMUS Conference Interpreter &bull; Canryn Production &bull; ${date}</div></body></html>`;
    ...
    if (printWindow) {
      ...
      toast.success('PDF transcript ready to print/save');
    } else {
      toast.error('Pop-up blocked. Please allow pop-ups to export PDF.');
    }
  }, [transcript, sourceLang, targetLang, contextLabel]);
  ...
  return (
    <div className={containerClass} role="region" aria-label="Language Interpreter">
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-800/50 shrink-0">
        ...
        <button onClick={exportTranscript} disabled={transcript.length === 0} className="p-2 rounded-lg bg-white/5 text-white/50 hover:bg-white/10 hover:text-white disabled:opacity-30" aria-label="Export transcript as text" title="Export as TXT">
          <Download className="w-4 h-4" />
        </button>
        <button onClick={exportTranscriptPDF} disabled={transcript.length === 0} className="p-2 rounded-lg bg-white/5 text-white/50 hover:bg-white/10 hover:text-white disabled:opacity-30" aria-label="Export transcript as PDF" title="Export as PDF">
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><text x="12" y="17" textAnchor="middle" fontSize="7" fill="currentColor" stroke="none" fontWeight="bold">PDF</text></svg>
        </button>
      </div>
