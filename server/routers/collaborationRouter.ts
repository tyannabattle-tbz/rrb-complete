...
export const collaborationRouter = router({
  ...
  // Create collaboration session for co-broadcasting
  createCollaborationSession: protectedProcedure
    .input(
      z.object({
        ...
        allowChatModeration: z.boolean().default(true),
      })
    )
    .mutation(async ({ ctx, input }) => {
      ...
      return {
        ...
        allowChatModeration: input.allowChatModeration,
        createdAt: new Date().toISOString(),
      };
    }),

  // Invite guest to collaboration
  ...
  // Mute all guests
  muteAllGuests: protectedProcedure
    .input(z.object({ sessionId: z.string() }))
    .mutation(async ({ input }) => {
      return {
        success: true,
  ...
  // Start screen share
  startScreenShare: protectedProcedure
    .input(
      z.object({
        sessionId: z.string(),
        collaboratorId: z.number(),
        screenType: z.enum(['desktop', 'window', 'tab']),
      })
    )
    .mutation(async ({ input }) => {
      const streamId = `screen_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      return {
        success: true,
        streamId,
        collaboratorId: input.collaboratorId,
